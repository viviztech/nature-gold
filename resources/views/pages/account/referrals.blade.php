        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="p-6">
                ...
                <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                    {{-- Code Display --}}
                    <div class="flex items-center gap-3 bg-gold-50 border-2 border-dashed border-gold-300 rounded-xl px-6 py-4">
                        <span class="font-heading text-2xl font-bold text-gold-700 tracking-widest" id="referral-code">{{ $user->referral_code }}</span>
                        <button onclick="copyCode()" class="p-2 text-gold-600 hover:text-gold-800 hover:bg-gold-100 rounded-lg transition" title="{{ __('shop.copy') }}">
                    ...
                    @foreach($referrals as $referral)
                        <div class="p-6 flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 bg-nature-50 rounded-full flex items-center justify-center">
                                    <span class="text-sm font-bold text-nature-700">{{ strtoupper(substr($referral->referred->name, 0, 1)) }}</span>
                                </div>
                                <div>
                                    <p class="font-semibold text-gray-800 text-sm">{{ $referral->referred->name }}</p>
